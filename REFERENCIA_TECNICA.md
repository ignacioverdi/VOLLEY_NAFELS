...
| | NÄFELS | CASLA |
|---|---|---|
| Repo | `VOLLEY_NAFELS` | `Voley-Stats` |
| Vercel | `volley-nafels` | `voley-stats-iota` |
| `firebase.js` | ~22 KB | ~15 KB |
| Sesión guardada como | `nla_sesion` | `casla_sesion` |
| Roles de usuario | sí (`coach`, `at`, `player`) | no tiene |
| Entrega la llave del cifrado | sí | no (no está cifrado) |
| Título del panel | "Panel en Vivo — Scouting de partido" | "Scout en Vivo — CASLA" |
...
`nla_players_db.json` (~14 MB) — toda la NLA suiza:
