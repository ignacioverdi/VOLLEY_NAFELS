...
if exist "update_db_nafels_FULL.py" (echo OK update_db_nafels_FULL.py >> "%LOG%") else (echo FALTA update_db_nafels_FULL.py >> "%LOG%")
...
dir /b /ad "DVW NAFELS *" >> "%LOG%" 2>&1
echo. >> "%LOG%"
set "DVW_DIR="
for /f "delims=" %%D in ('dir /b /ad "DVW NAFELS *" 2^>nul') do (
...
python update_db_nafels_FULL.py --dvw_dir "%DVW_DIR%" --temporada 2026 --output_dir . >> "%LOG%" 2>&1
echo. >> "%LOG%"
echo ===== FIN ===== >> "%LOG%"
type "%LOG%"
echo.
