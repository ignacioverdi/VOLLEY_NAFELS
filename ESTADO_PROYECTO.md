# 📋 ESTADO DEL PROYECTO — NÄFELS · CASLA · KIT DE CLIENTES
...
| | Dónde | Estado |
|---|---|---|
| **NÄFELS** | `ignacioverdi/VOLLEY_NAFELS` → Vercel `volley-nafels` | Blindado y cifrado |
| **CASLA** | `ignacioverdi/Voley-Stats` → Vercel `voley-stats-iota` | Protegido, sin cifrar (decisión propia) |
...
**Hallazgo grave:** en CASLA cualquier usuario registrado podía escribir en **toda** la
...
como `oe`. El generador contemplaba `Näfels` y `Nafels` pero no `Naefels`;
`Schönenwerd` pero no `Schoenenwerd`. Por esa letra **el club aparecía en
...
que cambie el Hub de NÄFELS, el generador vuelve a enlazar todo.
...
**CASLA no se cifra** — se termina la temporada y no se comercializa.
...
- **CASLA ≠ NÄFELS.** Distinto `firebase.js` (15 KB vs 22 KB), distinto nombre de
  sesión (`casla_sesion` vs `nla_sesion`), CASLA no tiene roles. **Nunca copiar y pegar
...
- Los `.dvw` nuevos van en `DVW NAFELS 2027`, no en la carpeta 2026.
...
**Técnicos, menores**
- `casla_players_dbBACKUP.json` — 9 MB, copia suelta. No la agarraría el cifrado porque
  el nombre termina en `BACKUP`.
- Borrar `temporadas/2025-26/_RESPALDO` en NÄFELS.
...
```
/llave                          la llave del cifrado (NÄFELS)
...
**UID de Nacho en CASLA:** `UqL6LAAkOdXSdCTBIZM6qYUY5hF3`
(el de NÄFELS es distinto — cada proyecto de Firebase asigna el suyo)
