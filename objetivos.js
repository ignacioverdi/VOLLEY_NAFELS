...
/* ══ LAS BATERIAS VIVEN EN objetivos_config.js ══════════════════════════════
   ...
   entre si y mas cortas que la buena: les faltaba todo lo que se agrego
   despues —la ventanita del desglose, el enlace al video, las traducciones,
   la marca de que fila es cada bateria—.

   Y cual ganaba dependia del ORDEN DE CARGA de cada pantalla:

       analisis.html     cargaba utils.js DESPUES      -> usaba la vieja
       panel_voley.html  cargaba objetivos.js DESPUES  -> usaba la vieja
       historial_voley   tenia la suya propia          -> usaba la vieja

   Por eso en esas pantallas la bateria no se podia tocar y no abria nada,
   mientras en el dashboard si. No era un bug de cada pantalla: era esta
   copia pisando a la buena.

   Ahora la definicion vive en UN SOLO LUGAR. Aca solo queda el reenvio, para
   que cualquier pantalla que llame a objSingleBat siga funcionando sin
   importar en que orden cargue sus archivos. */
function objSingleBat(id, val, meta, cls, objLine, vals){
  ...
  /* Si objetivos_config.js no esta cargado, se avisa en vez de dibujar una
     bateria a medias que despues nadie entiende por que no responde. */
   ...
   objetivo. */
        ...
        try{ var _q = (row && row.label) ? String(row.label) : '';
          ...
          __jug: (row && (row.jug || row.nombre || row.name)) || (/equipo/i.test(_q) ? '' : (window._dbatNombre || window._objNombre || ''))});
                ...
                return objSingleBat(id,val,_m,cls,objLine,row.vals);
      }).join('')+'</div>';
  });
  html+='</div>';
  el.innerHTML=html;
